@auth
    <form action="{{ route('carrinho.checkout') }}" method="post" class="mt-4">
        ...
        <div class="form-outline form-white mb-4">
            ...
            <input class="form-control form-control-lg @error('nif') is-invalid @enderror" type="number" name="nif"
                id="typeName" value="{{ old('nif', Auth::user()->cliente ? Auth::user()->cliente->nif ?? '' : '') }}"
                placeholder="756231562" />
            @error('nif')
        ...
        @else
            {{ __('You need to be a client to proceed to checkout') }}
        @endcan
