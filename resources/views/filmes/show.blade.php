                            ...
                            <ul class="nav nav-tabs" role="tablist">
                                @foreach ($conj_sessoes as $tipo_sessoes => $sessoes)
                                    <li class="nav-item"><a class="nav-link {{ $loop->first ? 'active' : '' }}"
                                            id="{{ $loop->index }}-tab" data-toggle="tab"
                                            href="#tab-{{ $loop->index }}" role="tab"
                                            aria-controls="tab-{{ $loop->index }}"
                                            aria-selected="{{ $loop->first ? 'true' : 'false' }}">5
                                            {{ $tipo_sessoes }}</a></li>
                                @endforeach
                            </ul>
                        </div>
                ...
                <div class="tab-content" id="myTabContent">
                    @foreach ($conj_sessoes as $sessoes)
                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }} "
                            id="tab-{{ $loop->index }}" role="tabpanel" aria-labelledby="{{ $loop->index }}-tab">
                            <div class="row">
                                <div class="col-12">
                                    <!-- accordion -->
                                                ...
                                                <div class="accordion__card">
                                                    <div class="card-header" id="heading{{ $loop->index }}">
                                                        <button type="button" data-toggle="collapse"
                                                            ...
                                                            @endif
                                                            <span>{{ date('d F Y', strtotime($sessao->data)) }},
                                                                {{ date('H:i', strtotime($sessao->horario_inicio)) }}</span>
                                                        </button>
                                                    </div>
                                                    ...
                                                    <div id="collapse{{ $loop->index }}"
                                                        ...
                                                        data-parent="#accordion{{ $loop->parent->index }}">
                                                        <div class="card-body">
                        ...
                        <div class="card">
                            ...
                            <div class="card__content">
                                <h3 class="card__title"><a
                                        href="{{ route('filmes.show', $destaque->id) }}">{{ $destaque->titulo }}</a></h3>
